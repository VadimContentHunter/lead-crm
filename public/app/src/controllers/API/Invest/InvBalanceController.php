...
class InvBalanceController
{
    ...
    private function initMethodMap(): void
    {
        if ($this->appContext instanceof ISecurity) {
            /**
             * @var InvBalanceController $secureCall
             */
        ...
        $this->methods = [
            'invest.balance.get' => fn() => $secureCall->getBalance($this->rpc->getParams()),
            'invest.balance.update' => fn() => $secureCall->createOrUpdateInvBalance($this->rpc->getParams()),
        ];
    }
